...
class WPSpin:
    ...
    def generate(self, algo, mac):
        ...
        if algo not in self.algos: raise ValueError('Invalid algorithm')
    ...
    def getSuggested(self, mac):
        ...
        for ID in algos:
            algo = self.algos[ID]
            item = {'id': ID, 'pin': self.generate(ID, mac)}
            item['name'] = ('Static PIN — ' if algo['mode'] == self.ALGO_STATIC else '') + algo['name']
            res.append(item)
        return res
    ...
    def _suggest(self, mac):
        ...
        for algo_id, masks in algorithms.items():
            if mac.startswith(masks): res.append(algo_id)
        return res
    ...
    def pinASUS(self, mac):
        ...
        pin = ''
        for i in range(7): pin += str((b[i % 6] + b[5]) % (10 - (i + b[1] + b[2] + b[3] + b[4] + b[5]) % 7))
        return int(pin)
